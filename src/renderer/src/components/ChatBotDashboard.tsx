...
const StatusBadge = styled.span<{ status: 'success' | 'warning' | 'info' }>`
  background: ${props => 
    props.status === 'success' ? 'rgba(40, 167, 69, 0.2)' : 
    props.status === 'warning' ? 'rgba(255, 193, 7, 0.2)' : 'rgba(0, 180, 216, 0.2)'};
  color: ${props => 
    props.status === 'success' ? '#2ecc71' : 
    props.status === 'warning' ? '#f1c40f' : '#00b4d8'};
...
const ChatBotDashboard: React.FC = () => {
  ...
  // Datos de actividad simulados para los departamentos
  const recentActivity = [
    { id: 1, user: '+54 9 11 5432-1098', intent: 'Consulta de Envío / Logística', time: 'Hace 2 min', status: 'success' as const },
    { id: 2, user: '+57 310 876 5432', intent: 'Estado de Pedido #4820', time: 'Hace 5 min', status: 'success' as const },
    { id: 3, user: '+52 55 1234 5678', intent: 'Soporte Técnico / Convenios', time: 'Hace 12 min', status: 'warning' as const },
    { id: 4, user: '+54 9 351 987 6543', intent: 'Horarios y Ubicación', time: 'Hace 18 min', status: 'success' as const },
    { id: 5, user: '+58 412 345 6789', intent: 'Requisición de Materiales', time: 'Hace 25 min', status: 'info' as const },
  ];
      ...
      <GridKpis>
        ...
        <KpiCard>
          ...
          <KpiInfo>
            ...
            <KpiLabel>Tasa de Automatización (Bot)</KpiLabel>
          </KpiInfo>
        </KpiCard>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <StatusBadge status={item.status}>
                      {item.status === 'success' ? 'Completado' : item.status === 'warning' ? 'Derivado a Humano' : 'En Proceso'}
                    </StatusBadge>
                  </td>
                </tr>
              ))}
