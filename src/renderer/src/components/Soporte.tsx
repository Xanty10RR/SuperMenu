...
const NodeContainer = styled(animated.div)<{
  $isProblem: boolean
  $isSolution: boolean
  $color: string
}>`
  ...
  color: ${(props) =>
    props.$color === '#000000' ? '#ffffff' : props.$color === '#ffffff' ? '#000000' : '#ffffff'};
  cursor: move;
  border: ${(props) =>
    props.$isProblem ? '2px solid #EF4444' : props.$isSolution ? '2px solid #10B981' : 'none'};
...
export const MindMap: React.FC = () => {
  ...
  const handleNodeDrag = (id: number, xy: [number, number]): void => {
    ...
    setNodes(
      nodes.map((node) => (node.id === id ? { ...node, position_x: x, position_y: y } : node))
    )
  }
  ...
  const handleDeleteNode = async (id: number): Promise<void> => {
    try {
      ...
      setConnections(
        connections.filter((conn) => conn.source_node_id !== id && conn.target_node_id !== id)
      )
    } catch (err) {
  ...
  return (
    <MindMapContainer>
      ...
      <AddNodeForm>
        ...
        <Button onClick={handleAddNode}>Agregar {selectedNode ? 'Subnodo' : 'Nodo Raíz'}</Button>
        {selectedNode && <Button onClick={() => setSelectedNode(null)}>Cancelar Selección</Button>}
      </AddNodeForm>
    </MindMapContainer>
  )
}
